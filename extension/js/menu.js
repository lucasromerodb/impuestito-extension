 ...
 */
function initMenu(text) {
  chrome.storage.sync.get(['data', 'userConfig'], function(result) {

    // If no result, try to get data again
    if(!result) {
    ...
    if (result.data) {
      const provinces = Object.fromEntries(
        Object.entries(result.data.province).sort(([,a], [,b]) => a.name.localeCompare(b.name))
      )
